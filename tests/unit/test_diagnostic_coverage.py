...
All error/warning directories are now backfilled (Batches A/B/C). The remaining
gap is exactly the 5 tests below, each of which has no honest code to assert.
BASELINE may only go DOWN -- lower it when one of these is resolved.

Known-bug exceptions (issues filed, fixes deferred):
- tests/control_flow/test_err_break_continue_outside_loop -- break/continue outside
  a loop hits a backend assert instead of emitting CE1003.
