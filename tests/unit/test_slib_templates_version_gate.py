"""The consumer refuses a binary .slib whose templates schema is not 5 (decision B).
...
refused with CE3512 and must be rebuilt. A SOURCE library recompiles from its units
and reads none of this, so the gate skips it.
"""
import pytest

from sushi_lang.backend.library_errors import LibraryError
from sushi_lang.compiler.pipeline import _check_library_templates_version
...
def test_a_version_5_binary_library_passes():
    _check_library_templates_version(_metadata("binary", 5), "new.slib")
