...
- Reference element types (`...&peek T`) and dynamic-array element types (`...T[]`) are rejected
  (**CE0114**).
...
| Code | Meaning |
| --- | --- |
| **CE0114** | `...T` must be last, at most one per function; element type must not be a reference or a dynamic array (`...T[]`); also rejected in generic functions |
...
- **Plain functions only** — packs (and `...T`) are not allowed in perk or extension methods
  (CE0115).
- **No spread / forwarding** (`f(arr...)`) and **no pack indexing** (`args.0`) — deferred.
