...
_add(ErrorMessage("CE2400", Severity.ERROR,
    ...
    Category.BORROW, "A borrow takes the address of storage a frame owns, so its target must be a local -- a parameter, a `let`, or a binding. A constant, a top-level function, an enum type name and an FFI namespace all name something else, and none of them has a frame slot to point at. Read the value instead, or copy it into a local first. A name that is declared NOWHERE is CE1001; this code is only for a name that exists and is not a local, which is why the two are no longer reported together for one token. ONE gate answers it for every position -- semantics/constant_borrow.py:reject_borrow_of_constant -- because the rule was written five times with four predicates and one of them read the FLAT constant table (#685). ConstantTable.by_name holds one record per NAME over the whole program and is first-wins, so a unit whose own `var` shared a name with an earlier unit's `const` was refused, and a unit whose own `const` shared a name with an earlier unit's `var` was let through to write read-only memory. The `poke` pattern binding was the position nothing asked at all, and it wrote there too. tests/unit/test_borrow_of_constant_matrix.py is the CI gate."))
