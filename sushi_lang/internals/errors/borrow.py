...
_add(ErrorMessage("CE2400", Severity.ERROR,
    ...
    Category.BORROW, "This is a WRITE rule. A constant is read-only STORAGE: it is one object in `.rodata`, so a pointer into it exists and `peek` reads through it. `poke` writes through the pointer, and a write to read-only memory is undefined behaviour and not a diagnostic; `nom` takes the value away, and a constant has no owner to take it from. Declare a `var` for storage that you can write and hand away. The code has a second half: a name that reaches storage of NO kind -- a top-level function, a registry stdlib constant, an FFI namespace -- refuses every mode, `peek` too, because there is nothing to point at. A name that is declared NOWHERE is CE1001 and a TYPE name in a value position is CE2105, so neither is reported together with this one for one token. ONE gate answers the question for every position -- semantics/constant_borrow.py:reject_borrow_of_constant -- because the rule was written five times with four predicates and one of them read the FLAT constant table (#685). ConstantTable.by_name holds one record per NAME over the whole program and is first-wins, so a unit whose own `var` shared a name with an earlier unit's `const` was refused, and a unit whose own `const` shared a name with an earlier unit's `var` was let through to write read-only memory. The `poke` pattern binding was the position nothing asked at all, and it wrote there too. Until #713 the code refused a `peek` as well, in three of its four positions, and the match arm allowed it: the reason line said a constant has no FRAME slot, which is true and is the wrong test, because nothing here needs a frame slot and the match arm already took the address. tests/unit/test_borrow_of_constant_matrix.py is the CI gate."))
