...
_add(ErrorMessage("CE2436", Severity.ERROR,
    "cannot move '{name}': it is {kind}, storage the program keeps for its whole run; borrow it instead, or take an independent value",
    Category.BORROW, "This is the rule for UNIT-LEVEL STORAGE, and both kinds read it. A `var` declaration is storage in the data segment and a `const` is one object in `.rodata`: one per program, initialized before `main`, never destroyed at exit (docs/design/unit-storage.md). Neither has an owner that can hand the value away. Moving it out -- a `nom` argument, a `let` bound straight from it, a `return` of it, a `nom self` method such as `close()` -- would give a callee or a binding the right to free storage nothing re-initializes, so the same rule that fences `main`'s argv view (CE2410) fences both. A plain value copies out freely; only a type that owns a resource is refused, which is why `nom` of a `const i32` stays the harmless copy it always was. Pass it as a borrow (`f(v)`, `peek v`, `poke v`), or take an independent value: `.clone()` for a plain owner, `.share()` for a handle. A REBIND is the one way to change what a `var` holds, and it frees the old value. The constant half arrived with #726: a take of a constant compiled and ran, because a `nom` is a CONSUMING use and the borrow pass returned early for a name with no `BorrowState`. It was harmless by accident alone -- a constant could hold nothing but plain data and a literal-backed `string`, whose drop is a no-op -- and the constant surface has widened repeatedly. The home is the borrow pass because every other consuming-use rule is there (CE2405, CE2410, CE2411, CE2401); CE2400 stays the WRITE rule."))
...
_add(ErrorMessage("CE2400", Severity.ERROR,
    "cannot borrow '{name}' with `{mode}`",
    Category.BORROW, "This is a WRITE rule. A constant is read-only STORAGE: it is one object in `.rodata`, so a pointer into it exists and `peek` reads through it. `poke` writes through the pointer, and a write to read-only memory is undefined behaviour and not a diagnostic. Declare a `var` for storage that you can write. A TAKE is not this code's: `nom` is a consuming use, so CE2436 answers it for a constant and a unit variable alike (#726). The code has a second half: a name that reaches storage of NO kind -- a top-level function, a registry stdlib constant, an FFI namespace -- refuses every mode, `peek` too, because there is nothing to point at. A name that is declared NOWHERE is CE1001 and a TYPE name in a value position is CE2105, so neither is reported together with this one for one token. ONE gate answers the question for every position -- semantics/constant_borrow.py:reject_borrow_of_constant -- because the rule was written five times with four predicates and one of them read the FLAT constant table (#685). ConstantTable.by_name holds one record per NAME over the whole program and is first-wins, so a unit whose own `var` shared a name with an earlier unit's `const` was refused, and a unit whose own `const` shared a name with an earlier unit's `var` was let through to write read-only memory. The `poke` pattern binding was the position nothing asked at all, and it wrote there too. Until #713 the code refused a `peek` as well, in three of its four positions, and the match arm allowed it: the reason line said a constant has no FRAME slot, which is true and is the wrong test, because nothing here needs a frame slot and the match arm already took the address. tests/unit/test_borrow_of_constant_matrix.py is the CI gate."))
...
_add(ErrorMessage("CE2426", Severity.ERROR,
    "cannot write to '{name}': it borrows storage another value still owns",
