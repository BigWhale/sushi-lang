...
_add(ErrorMessage("CE4013", Severity.ERROR,
    ...
    Category.PERK, "A perk implementation is keyed by the name of the type it targets, and a generic target's key carries the TYPE-PARAMETER names -- `Box<T>`, which no concrete instance matches. So `extend Box@(T) with Drop` would register an implementation that never fires, and a value that looks like it releases a resource would silently leak it. A silent no-op is worse than a refusal, so v1 refuses. Implement `Drop` on each concrete instantiation that needs it, or let the generic's owning FIELDS carry the ownership -- a `BufReader@(R)` holding an owning R needs no Drop of its own, because destroying its fields destroys the handle."))
