...
_add(ErrorMessage("CE3011", Severity.ERROR,
    ...
    Category.UNIT, "A TYPE is one name for the whole program. Identity is nominal, so one name is one shape, and a source library's units and a bundled stdlib module are ordinary compilation units at the consumer: a struct or an enum that either of them declares is a name the consumer cannot declare again, even where the library keeps it private and the consumer cannot see it. The compiler used to let the consumer's declaration replace the library's without a word. The replacement was never registered, so the consumer also lost its own declaration, and then heard CE3005 about a private it wrote itself, or CE2027 about a struct shape it never spelled. This code refuses the declaration and names the unit that holds the name. A BINARY library holds the same rule: a concrete type it exports (#739) and a type it keeps PRIVATE (#761) are both refused with this code, because the library's bodies were compiled against its own layout, and the analysis stops after the refusal so no body is checked against the consumer's type. The FUNCTION arm retired with unit namespaces: a function carries the unit that declared it and each unit reads its own, so a consumer may declare a function beside a library's private one. Rename your type. `docs/design/type-identity.md` phase 2 -- an interned name that carries its unit -- is what would lift this."))
