...
def emit_name(codegen: 'LLVMCodegen', expr: Name, to_i1: bool) -> ir.Value:
    ...
    # Try local variables. Only find_local_slot's own KeyError means "not a local";
    # narrowing to it keeps a genuine failure inside the load from being silently
    # mis-routed into the function-pointer path below.
    try:
    ...
    # Not a local: a bare reference to a top-level function is a first-class function
    # value -> a non-capturing fat pointer {thunk, null, null}. The thunk bridges the
    # bare fn into the uniform env-passing indirect ABI.
